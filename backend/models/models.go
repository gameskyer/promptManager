...
func (pv *PresetVersion) ToSnapshotData() (*SnapshotData, error) {
	...
	fmt.Printf("[DEBUG] ToSnapshotData: Snapshot=%v\n", pv.Snapshot)

	data := &SnapshotData{}
	if posText, ok := pv.Snapshot["pos_text"].(string); ok {
		data.PosText = posText
		fmt.Printf("[DEBUG] ToSnapshotData: pos_text found=%s\n", posText)
	} else {
		fmt.Printf("[DEBUG] ToSnapshotData: pos_text not found or not string, type=%T\n", pv.Snapshot["pos_text"])
	}
